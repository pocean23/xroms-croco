...
def test_sel2d():
    ...
    assert (
        xroms.sel2d(ds.u, ds.lon_u, ds.lat_u, lon0, lat0).isel(s_rho=0, time=0)
        == 0.7
    )
    assert np.allclose(
        xroms.sel2d(ds.v, ds.lon_v, ds.lat_v, lon0, lat0).isel(s_rho=0, time=0),
        0.042857,
    )
...
#     assert np.allclose(xroms.sel2d(ds.u, ds.lon_u, ds.lat_u, lon0, lat0).isel(s_rho=0, time=0), [0.1, 0.7])
#     assert np.allclose(xroms.sel2d(ds.v, ds.lon_v, ds.lat_v, lon0, lat0).isel(s_rho=0, time=0), [-0.071429,  0.042857])
